...
# class _Operate(BaseOperate,_Base):
#     __tablename__ = 'operate_his1'
# def new_operate():
#     _session = _Session()
#     session = Session()
#
#     for col in _session.query(_Operate).filter(_Operate.GET_ON_LATITUDE != 0).filter(_Operate.GET_ON_LATITUDE != 0).all():
#         col = Operate(ID=col.ID,WORK_BEGIN_TIME=col.WORK_BEGIN_TIME,GET_ON_LONGITUDE=col.GET_ON_LONGITUDE,
#                        GET_ON_LATITUDE=col.GET_ON_LATITUDE,GEOHASH5=col.GEOHASH5,GEOHASH7=col.GEOHASH7)
#         session.add(col)
#     session.commit()
#     return True
...
if __name__ =='__main__':
    new_muti_operate(range(3,5))    #或者(3,4,5)，传入一个可迭代对象